...
function wccm_launch() {
	...
	if ( is_admin() ) {
		...
		if ( intval( get_option( 'wccm_compare_page' ) ) > 0 ) {
			...
			require_once 'includes/wishlist-page.php';
			require_once 'includes/catalog.php';

			add_action('wp_head', function(){
				echo "<script>window.wcwl_url = '/wp-content/plugins/wc-compare-wishlist/includes/actions.php'; </script>";
			    echo "<script>window.wc_compare_class = '".get_option('wccm_compare_button_class')."'; </script>";
			    echo "<script>window.wc_wishlist_class = '".get_option('wccm_wishlist_button_class')."'; </script>";
			    echo "<script>window.wc_compare_page_url = '".get_permalink(get_option('wccm_compare_page'))."'; </script>";
			    echo "<script>window.wc_wishlist_page_url = '".get_permalink(get_option('wccm_wishlist_page'))."'; </script>";
			});
		}
	}
...
function wccm_enqueue_style() {
	wp_enqueue_style( 'wccm-style', plugins_url( 'css/wccm-style.css', __FILE__ ), array(), time() ); //WCCM_VERISON
	wp_enqueue_script( 'wccm-ajax-actions', plugins_url( 'js/ajax-actions.js', __FILE__ ), array(), time() );
