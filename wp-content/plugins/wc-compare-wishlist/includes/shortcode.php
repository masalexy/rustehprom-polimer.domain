...
function wccm_wish_list_render( $list, $attributes = array() ) {
	$products = array();
	foreach ( $list as $product_id ) {
		$product = wc_get_product( $product_id );
		if ( $product ) {
	...
	if ( !empty( $products ) ) {
		ob_start();
			echo '<div class="wccm-compare-table">';
				wccm_wish_list_render_header( $products );
			echo '</div>';
			$content = ob_get_contents();
		ob_end_clean();
	}
						...
						foreach ( $products as $product_id => $product ) {
							echo '<td class="wccm-td">';
								echo '<div class="wccm-thumb">';
									echo '<a class="dashicons dashicons-no" href="', wccm_get_compare_link( $product_id, 'remove-from-compare-list' ), '"></a>';
...
function wccm_wish_list_render_header( $products ) {
	echo "<div class='wishlist-products'>";
	    foreach ( $products as $product_id => $product ){
	        $remove_url = wccm_get_compare_link($product->get_id(), 'remove-from-wish-list');
	        echo "
	            <div class='wishlist-product'>
	                <div class='wishlist-product-remove'>
						<a href='{$remove_url}' class='dashicons dashicons-no'></a>
					</div>
	                <div class='wishlist-product-image'>{$product->get_image('thumbnail')}</div>
	                <div class='wishlist-product-title'>{$product->get_name()}</div>
	                <div class='wishlist-product-price'>{$product->get_price_html()}</div>
