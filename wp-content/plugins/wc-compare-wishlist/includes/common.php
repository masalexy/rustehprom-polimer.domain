...
function wccm_setup_plugin() {
	...
	if ( !is_admin() ) {
		...
		wp_register_script( 'wccm-compare', $base_path . 'js/compare.js', array( 'jquery' ), '1.0' );
...
function wccm_get_wish_list() {
	global $wccm_wish_list;

	if ( is_null( $wccm_wish_list ) ) {
		$wccm_wish_list = !empty( $_COOKIE['wish-list'] ) ? $_COOKIE['wish-list'] : '';
		if ( empty( $wccm_wish_list ) ) {
			$wccm_wish_list = '';
		}

		$wccm_wish_list = explode( ':', $wccm_wish_list );
		$nonce = array_pop( $wccm_wish_list );
		if ( !wp_verify_nonce( $nonce, implode( $wccm_wish_list ) ) ) {
			$wccm_wish_list = array();
		}

		sort( $wccm_wish_list );
	}

	return $wccm_wish_list;
}
...
function wccm_set_wish_list( $list ) {
	global $wccm_wish_list;
	...
	setcookie( 'wish-list', $value, 0, $path );
}
...
function wccm_get_wishlist_page_link( $items ) {
	$page_id = intval( get_option( 'wccm_wishlist_page' ) );
	if ( !$page_id ) {
		return false;
	}
