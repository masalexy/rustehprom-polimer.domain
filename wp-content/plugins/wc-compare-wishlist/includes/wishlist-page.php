...
add_action( 'wp_enqueue_scripts', 'wccm_enqueue_wishlist_scripts' );
add_action( 'template_redirect', 'wccm_set_wishlist_page_cookie' );
// register filter hooks
add_filter( 'the_content', 'wccm_render_wishlist_page' );
 ...
 */
function wccm_enqueue_wishlist_scripts() {
 ...
 */
function wccm_set_wishlist_page_cookie() {
	if ( is_page() && get_option( 'wccm_wishlist_page' ) == get_queried_object_id() ) {
		$list = get_query_var( wccm_get_endpoint(), false );
		if ( $list ) {
			$parsed_list = array_filter( array_map( 'intval', explode( '-', $list ) ) );
			if ( !empty( $parsed_list ) ) {
				$cookie = isset( $_COOKIE['wish-lists'] ) ? explode( ',', $_COOKIE['wish-lists'] ) : array();
				...
				setcookie( 'wish-lists', $value, $expire, $path );
			}
		}
	}
}
 ...
 */
function wccm_render_wishlist_page( $content ) {
	if ( !is_page() || get_option( 'wccm_wishlist_page' ) != get_the_ID() ) {
		return $content;
	}
	...
	if ( empty( $list ) ) {
		$list = wccm_get_wish_list();
		if ( empty( $list ) ) {
			return $content . '<p class="wccm-empty-compare">Нет товаров</p>';
		}
	}

	return $content . wccm_wish_list_render( $list );
}
